...
def logic(querydata, locationdata, max_results):
    ...
    youtube_object = build(YOUTUBE_API_SERVICE_NAME,
                           YOUTUBE_API_VERSION, developerKey=keys[0])
    ...
    def get_channel_statistics(youtube_object, keys):
        ...
        for tk in range(len(ids)):  # loop for each specific query
            for j in range(len(ids[tk])-2):  # loop over unique ids in the query
                print(ids[tk][len(ids[tk])-1], ids[tk]
                      [len(ids[tk])-2], ids[tk][j])
                channeldata = youtube_object.channels().list(
                    part="snippet,statistics", id=ids[tk][j])
